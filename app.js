 ...
 * WARNING: You MUST run this via a local server (e.g., VS Code Live Server) or GitHub Pages.
 */
...
const ProductDB = [
  ...
  { id: 'vh-04', name: 'Linnen Dekbedovertrek', price: '€ 189', img: 'https://images.unsplash.com/photo-1522771739844-6a9f6d5f14af?auto=format&fit=crop&w=600&q=80' },
  ...
  { id: 'vh-06', name: 'Wollen Vloerkleed', price: '€ 450', img: 'https://images.unsplash.com/photo-1600607686527-6fb886090705?auto=format&fit=crop&w=600&q=80' }
];
...
class VerdeCartDrawer extends HTMLElement {
    ...
    this.innerHTML = `
      ...
      <div id="drawer-panel" class="fixed top-0 right-0 h-full w-full max-w-md bg-white z-50 transform translate-x-full transition-transform duration-300 shadow-2xl flex flex-col">
        <div class="p-6 border-b border-gray-100 flex justify-between items-center bg-white">
          <h2 class="text-xl font-bold text-gray-900">Winkelwagen</h2>
          <button id="close-drawer" class="text-gray-500 hover:text-gray-900 text-3xl font-light">&times;</button>
        </div>
        <div id="cart-items" class="flex-grow p-6 overflow-y-auto space-y-4 bg-gray-50">
          <p class="text-gray-400 text-center mt-10 font-medium">Je winkelwagen is leeg.</p>
        </div>
        <div class="p-6 border-t border-gray-100 bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
          ...
          <button class="w-full bg-gray-900 text-white py-4 rounded font-semibold hover:bg-gray-800 active:scale-[0.98] transition-all">Veilig Afrekenen</button>
        </div>
      </div>
    `;
  ...
  renderItems(items) {
    ...
    container.innerHTML = items.map(item => `
      <div class="flex gap-4 items-center bg-white p-4 border border-gray-100 rounded-lg shadow-sm">
...
class ViewHome extends HTMLElement {
  connectedCallback() {
    this.innerHTML = `
      <section class="relative w-full h-[80vh] min-h-[600px] flex items-center justify-center overflow-hidden bg-gray-900">
        ...
        <div class="relative z-10 text-center px-4 max-w-4xl mx-auto flex flex-col items-center">
          <h1 class="text-4xl md:text-6xl font-bold text-white mb-6 tracking-tight">Interieur dat niet ten koste gaat van de wereld.</h1>
          <p class="text-xl text-gray-200 mb-10 font-light">Bliksemsnel geleverd, levenslang plezier.</p>
          <a href="#collectie" class="inline-block px-10 py-4 bg-white text-gray-900 font-semibold rounded hover:bg-gray-100 transition-colors shadow-lg">Bekijk de Collectie</a>
        </div>
      </section>
      
      <section class="max-w-7xl mx-auto px-4 py-24">
        <div class="flex justify-between items-end mb-12 border-b border-gray-200 pb-4">
          <h2 class="text-3xl font-bold text-gray-900 tracking-tight">Uitgelicht</h2>
    ...
    ProductDB.slice(0, 3).forEach(p => {
      ...
      el.className = "group relative block overflow-hidden rounded-lg bg-gray-100 cursor-pointer shadow-sm hover:shadow-xl transition-shadow duration-300";
      el.innerHTML = `
        <img src="${p.img}" alt="${p.name}" class="w-full h-[450px] object-cover transition-transform duration-700 group-hover:scale-105">
        <div class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end justify-center pb-8">
          <button class="bg-white text-gray-900 px-8 py-3 rounded shadow-lg font-semibold transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 hover:bg-gray-50 active:scale-95">Snel Toevoegen</button>
        </div>
        <div class="absolute top-4 left-4 bg-white/95 px-3 py-1 rounded text-sm font-bold text-gray-900 shadow-sm">${p.price}</div>
        <div class="absolute bottom-4 left-4 right-4 text-white font-semibold text-lg drop-shadow-md z-10">${p.name}</div>
      `;
  ...
  connectedCallback() {
    this.innerHTML = `
      <div class="bg-gray-900 py-20 text-center px-4">
        ...
        <p class="text-gray-300 mt-4 max-w-2xl mx-auto text-lg font-light">Duurzame materialen, tijdloos design. Ontworpen om generaties lang mee te gaan.</p>
      </div>
    ...
    ProductDB.forEach(p => {
      const el = document.createElement('div');
      el.className = "group flex flex-col overflow-hidden";
      el.innerHTML = `
        <div class="relative overflow-hidden rounded-lg mb-4 bg-gray-100">
          <img src="${p.img}" alt="${p.name}" class="w-full h-[350px] object-cover transition-transform duration-500 group-hover:scale-105">
        </div>
        <h3 class="font-bold text-gray-900 text-lg">${p.name}</h3>
        <p class="text-gray-500 font-medium mb-4">${p.price}</p>
        <button class="mt-auto w-full bg-white border-2 border-gray-900 text-gray-900 py-3 rounded font-bold hover:bg-gray-900 hover:text-white transition-colors active:scale-95">In Winkelwagen</button>
      `;
  ...
  connectedCallback() {
    this.innerHTML = `
      <section class="max-w-4xl mx-auto px-4 py-32 text-center">
        ...
        <div class="bg-gray-900 text-white p-12 md:p-16 rounded-3xl shadow-2xl">
